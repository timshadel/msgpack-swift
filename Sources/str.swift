...
extension String: MsgPackValueType {
    ...
    static func unpack<G: GeneratorType where G.Element == UInt8>(inout generator: G, length: Int) throws -> String {
        let data = try NSData.unpack(&generator, length: length)
        guard let str = String(data: data, encoding: NSUTF8StringEncoding) else { throw MsgPackError.UnsupportedValue(data) }
        return str
    }
